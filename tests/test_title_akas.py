...
@pytest.mark.parametrize(("imdb_id", "n", "akas"), [
    ("tt0429489", 3, [  # A Ay
...
def test_title_akas_parser_should_set_akas(imdb_id, n, akas):
    ...
    if len(akas) > 0:
        assert [(aka.title, aka.country_code, aka.country, aka.language_code, aka.language,
                 aka.is_alternative, aka.notes)
                 for aka in parsed.akas] == akas


@pytest.mark.parametrize(("imdb_id", "akas_count"), [
